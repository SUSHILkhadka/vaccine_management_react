 ...
 * throws error
 */
const keyValueValidator = async (key: string, value: any, schema: yup.ObjectSchema<any>) => {
  schema.validateSyncAt(key, { [key]: value });
};

export const ruleForSignIn = {
  validator: async ({ field }: any, value: any) => await keyValueValidator(field, value, signupSchema),
};
...
export const ruleForVaccine = {
  validator: async ({ field }: any, value: any) => await keyValueValidator(field, value, vaccineSchema),
};

export const ruleForOldPassword = {
  validator: async ({ field }: any, value: any) => await keyValueValidator('password', value, signupSchema),
};
...
export const newPasswordRule = (getFieldValue: any, value: string) => {
  if (getFieldValue('oldPassword') !== value) {
    ...
    return Promise.reject(new Error('New password cannot be same as old password'));
  }
};
